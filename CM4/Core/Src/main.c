...
#define LED_ALL_OFF()    do { LED_GREEN_OFF(); LED_RED_OFF(); } while(0)
...
int main(void)
{
    ...
    RCC->AHB4ENR |= (1U << 1);
    BUSY(10000);
    GPIOB->MODER &= ~(3U << 0);   GPIOB->MODER |= (1U << 0);   /* PB0  out */
    GPIOB->MODER &= ~(3U << 28);  GPIOB->MODER |= (1U << 28);  /* PB14 out */
    ...
    for (int j = 0; j < 2; j++) {
        LED_GREEN_ON(); HAL_Delay(200U);
        LED_GREEN_OFF(); HAL_Delay(200U);
    }
    HAL_Delay(500U);
    ...
    LED_RED_ON(); HAL_Delay(1000U); LED_RED_OFF(); HAL_Delay(500U);
    ...
    for (int j = 0; j < 2; j++) {
        LED_RED_ON(); HAL_Delay(500U);
        LED_RED_OFF(); HAL_Delay(500U);
    }
    HAL_Delay(500U);
    ...
    for (int attempt = 0; attempt < 10; attempt++) {
        ...
        if (got_echo) {
            ...
            LED_GREEN_ON(); HAL_Delay(300U); LED_GREEN_OFF();
        } else {
            /* RED blink = no echo this attempt */
            LED_RED_ON(); HAL_Delay(300U); LED_RED_OFF();
        }
        HAL_Delay(200U);
    }
    HAL_Delay(1000U);
    ...
    while (1) {
        ...
        if (blinks > 20U)  { blinks = 20U; }

        for (uint32_t b = 0; b < blinks; b++) {
            LED_GREEN_ON(); HAL_Delay(150U);
            LED_GREEN_OFF(); HAL_Delay(150U);
        }
...
void Error_Handler(void)
{
    __disable_irq();
    while (1) {
        LED_RED_ON();  BUSY(500000);
        LED_RED_OFF(); BUSY(500000);
    }
}
